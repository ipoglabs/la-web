 ...
 *
 * Dedicated confirmation flow — opened from the Danger Zone banner on the profile page.
 * Handles 3 stages, all state lives in useDeleteAccountStore (Zustand):
 *
 * Stage 2 — Feedback : user selects reasons + optional free-text.
 *            Action   : submitFeedback() → [API] POST /account/delete-feedback
 * Stage 3 — Review   : user reads their reasons and ticks acknowledgement checkbox.
 *            Action   : deleteAccount() → [API] DELETE /account
 * Stage 4 — Goodbye  : warm confirmation screen.
 *
 * Navigation:
 * - Back on Stage 2 → router.back() (returns to entry point)
 * - Back on Stage 3 → Stage 2 (confirmed reset)
 * - "Start over" on Stage 4 → reset store + router.push (POC only)
 */
...
export default function DeleteAccountConfirmPage() {
  ...
  const {
    stage, setStage,
    reasons, toggleReason,
    details, setDetails,
    confirmed, setConfirmed,
    isLoading, error, clearError,
    submitFeedback, deleteAccount,
    reset,
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {stage === 2 && (
            <div className="px-6 py-7 sm:px-8 space-y-7">
              <div>
                <h2 className="text-lg font-medium text-slate-900">Before you go — what happened?</h2>
          ...
          {stage === 3 && (
            <div className="px-6 py-7 sm:px-8 space-y-7">
              <div>
                <h2 className="text-lg font-medium text-slate-900">One last check</h2>
              ...
              <div className="space-y-2.5">
                <p className="text-sm font-medium text-slate-700">Your reasons</p>
                ...
                {detailsCount > 0 && (
                  <div className="mt-6 space-y-2">
                    <p className="text-sm font-medium text-slate-700">Additional Details</p>
              ...
              <label className="flex cursor-pointer items-start gap-3 rounded-lg py-1 transition hover:opacity-80 focus-within:ring-2 focus-within:ring-blue-300">
                <input
                  ...
                  className="sr-only"
                />
                <div className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border transition ${confirmed ? "border-blue-500 bg-blue-500" : "border-slate-400 bg-white"}`}>
                  {confirmed && <Check className="h-2.5 w-2.5 text-white" strokeWidth={3} />}
                </div>
                <p className="text-sm font-normal text-slate-800">
                  I confirm my decision to delete my account. I understand this action is permanent and cannot be undone.
                </p>
              </label>
              ...
              <div className="flex items-center gap-3">
                <button
                  type="button"
                  onClick={() => { setStage(2); setConfirmed(false); clearError(); }}
                  className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 px-4 py-2.5 text-sm font-medium text-slate-600 transition hover:border-slate-300"
                >
                  <ArrowLeft className="h-4 w-4" />
                  Back
                ...
                <button
                  type="button"
                  disabled={!canProceedFromStage3 || isLoading}
                  onClick={deleteAccount}
                  className={`inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-sm font-medium transition ${
                    canProceedFromStage3 && !isLoading
