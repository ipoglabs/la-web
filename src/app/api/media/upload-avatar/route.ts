...
export async function POST(req: NextRequest) {
  ...
  // Fixed key per user — re-uploads overwrite the same object rather than
  // accumulating orphaned files. Callers cache-bust the returned URL.
  const key = `${userId}/avatar/avatar.jpg`;
  ...
  try {
    ...
    console.error("[R2 Avatar Upload] code:", code, "| message:", message);
