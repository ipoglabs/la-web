...
export function ListingLiveEmail(data: ListingLiveData): string {
  ...
  const content = `
<h1 style="${s({ ...emailText.h1, margin: "0 0 12px" })}">🎉 Your listing is live!</h1>
<p style="${s({ ...emailText.body, margin: "0 0 24px" })}">Great news &mdash; your listing is now live on LokalAds and visible to buyers in your area.</p>
<div style="${s({ backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 10, padding: "16px 20px", marginBottom: 24 })}">
  <p style="${s({ ...emailText.eyebrow, margin: "0 0 6px" })}">Your listing</p>
...
<div style="${s({ textAlign: "center", marginBottom: 28 })}">
  ${emailButton("View Your Listing", esc(fullUrl))}
</div>
...
<p style="${s({ ...emailText.disclaimer, margin: 0 })}">You can manage your listings anytime from <a href="${APP_URL}/myads" style="${s(emailText.link)}">My Ads</a>.</p>
`;
