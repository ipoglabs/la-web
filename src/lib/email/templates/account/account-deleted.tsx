...
export function AccountDeletedEmail(data: AccountDeletedData): string {
  const content = `
<h1 style="${s({ ...emailText.h1, margin: "0 0 12px" })}">Account deleted, ${esc(data.firstName)}</h1>
<p style="${s({ ...emailText.body, margin: "0 0 24px" })}">Your LokalAds account has been permanently deleted. All your data, listings, and saved information have been removed from our systems.</p>
...
<p style="${s({ ...emailText.body, margin: "0 0 24px" })}">We&#39;re sorry to see you go. If you ever change your mind, you&#39;re always welcome to create a new account at LokalAds.</p>
...
<p style="${s({ ...emailText.disclaimer, margin: 0 })}"><a href="${APP_URL}/support" style="${s(emailText.link)}">Contact Support</a> &middot; <a href="${APP_URL}" style="${s(emailText.link)}">Create a New Account</a></p>
`;
