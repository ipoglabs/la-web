...
export function ReportTicketUpdatedEmail(data: ReportTicketUpdatedData): string {
  ...
  const labelStyle = s({ ...emailText.tableLabel, paddingBottom: 12, paddingRight: 16, verticalAlign: "middle", width: 80 });
  const valueStyle = s({ fontSize: 14, color: "#334155", paddingBottom: 12, verticalAlign: "middle", lineHeight: 1.4 });
  const badgeStyle = s({ ...emailText.badge, display: "inline-block", backgroundColor: status.bg, color: status.color, border: `1px solid ${status.border}`, borderRadius: 100, padding: "2px 10px" });
  const resolutionRow = data.resolution
    ? `<tr><td style="${s({ ...emailText.tableLabel, paddingBottom: 0, paddingRight: 16, verticalAlign: "middle", width: 80 })}">Note</td><td style="${s({ fontSize: 14, color: "#334155", paddingBottom: 0, verticalAlign: "middle", lineHeight: 1.4 })}">${esc(data.resolution)}</td></tr>`
    : "";
  const content = `
<h1 style="${s({ ...emailText.h1, margin: "0 0 12px" })}">Your report has been updated</h1>
<p style="${s({ ...emailText.body, margin: "0 0 24px" })}">The status of your report has changed. Here are the latest details:</p>
...
<p style="${s({ ...emailText.disclaimer, margin: 0 })}">If you have questions about this report, contact us at <a href="${APP_URL}/support" style="${s(emailText.link)}">our support page</a> and reference ticket <strong>#${esc(data.ticketId)}</strong>.</p>
`;
